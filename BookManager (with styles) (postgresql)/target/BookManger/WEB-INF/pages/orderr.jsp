...
<c:if test="${!empty listBooks}">
    <table align="center"  class="tg">
        <tr>
            <th width="80">ID_ORDERR</th>
            <th width="120">ID_CLIENT</th>
            <th width="120">DATE</th>
            <th width="60">DELETE</th>
        </tr>
        <c:forEach items="${listBooks}" var="product">
            <tr>
                <td>${product.id_orderr}</td>
                <td>${product.id_client}</td>
                <td>${product.date}</td>
                <td><a href="<c:url value='/removeorderr/${product.id_orderr}'/>">Delete</a></td>
            </tr>
        </c:forEach>
    </table>
</c:if>

<h1><center>Add a order</center></h1>

<c:url var="addAction" value="/orderr/add"/>
...
<form:form action="${addAction}" commandName="book">
    <table align="center">
        ...
        <tr>
            <td>
                <form:label path="id_client">
                    <spring:message text="id_client"/>
                </form:label>
            </td>
            <td>
        ...
        <tr>
            <td colspan="2" align="center" >
                <c:if test="${!empty book.id_client}">
                    <input type="submit"
                           value="<spring:message text="Edit orderr"/>"/>
                </c:if>
                <c:if test="${empty book.id_client}">
                    <input type="submit"
                           value="<spring:message text="Add orderr"/>"/>
                </c:if>
            </td>
        </tr>
    </table>
